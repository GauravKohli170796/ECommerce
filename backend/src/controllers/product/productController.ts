...
export class ProductController {
  ...
  @Post("/addProduct")
  async addProduct(@MultipartFile("images") @Required images: PlatformMulterFile[] ,@BodyParams() @Required() product: IAddProductRequest) {
    const imagesUrl = await this.cloudinaryService.uploadResource(images);
    product.images = imagesUrl;
    product.productDetails=JSON.parse(product.productDetails as string);
    return this.productService.addProduct(product);
  }

  @Post("/uploadFile")
  uploadFile(@MultipartFile("images") @Required images: PlatformMulterFile[] ,@BodyParams() params: any) {
    const x=JSON.parse(params?.params);
    console.log(x.a);
